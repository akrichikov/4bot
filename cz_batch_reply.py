...
class CZReplyGenerator:
    ...
    def _load_cz_prompt(self) -> str:
        ...
        if prompt_path.exists():
            with open(prompt_path, 'r', encoding='utf-8') as f:
                content = f.read()
                logger.info("✅ Loaded CZ persona prompt")
                return content
        logger.warning("⚠️ CLAUDE.md not found, using default responses")
        return ""
    ...
    def generate_reply(self, post: Post) -> str:
        ...
        # Check for FUD
        fud_terms = ['scam', 'rug', 'ponzi', 'crash', 'dead', 'fraud', 'fake', 'dump', 'worthless']
        ...
        build_terms = ['build', 'buidl', 'ship', 'launch', 'develop', 'create', 'code', 'deploy']
        if any(term in content_lower for term in build_terms):
            responses = [
                "This is the way! Keep BUIDLing 🚀",
                "Love to see builders building. LFG!",
                "Exactly right. We build through everything.",
                "Building is the answer. Always has been.",
                "This mindset wins. Keep pushing forward.",
                "Focus on building, ignore the noise. 🔨",
            ]
        ...
        if '?' in post.content:
            if 'when' in content_lower:
                responses = [
                    ...
                    "Timing the market < Time in the market. Keep building.",
                    "Focus on building value, not timing. Long term wins.",
                ]
            elif 'how' in content_lower:
                responses = [
                    "Start small, learn constantly, build consistently.",
                    "One step at a time. The path becomes clear when you start walking.",
                    "By doing. Start building, learn as you go.",
                ]
            elif 'why' in content_lower:
                responses = [
                    "Because we're building the future of finance.",
                    "Why not? Someone has to build it. Might as well be us.",
                    "Because centralized systems have failed too many people.",
                ]
            else:
                responses = [
                    "Great question! The answer is always: keep building.",
                    "Focus on what you can control. BUIDL.",
                    "Think long term. Build for the future.",
                ]
            return random.choice(responses)

        # Check for price/market talk
        if any(term in content_lower for term in ['price', 'chart', 'pump', 'moon', 'bear', 'bull']):
            responses = [
                "Less charts, more building.",
        ...
        # For mentions or direct replies
        if post.is_mention or post.is_reply:
            responses = [
                "Appreciate you! Let's keep building together 🚀",
                "This is the energy we need. Keep pushing!",
                "Together we BUIDL the future.",
                "Love the support! Onwards and upwards.",
        ...
        responses = [
            ...
            "This is the way. Stay focused.",
            "Every day we build the future.",
            "Winners focus on winning. Losers focus on winners.",
            "Less noise, more signal. Less talk, more build.",
            "The work continues. BUIDL.",
...
async def main():
    ...
    # Save log
    log_path = Path("/Users/doctordre/projects/4bot/logs/cz_batch_log.json")
